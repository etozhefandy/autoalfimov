...
def is_account_active(account_id):
    try:
        status = AdAccount(account_id).api_get(fields=['account_status']).get('account_status')
...
def get_facebook_data(account_id, date_preset, date_label=''):
    ...
    if account_id in LEAD_FORM_ACCOUNTS:
        leads = actions.get('offsite_conversion.fb_pixel_submit_application', 0) or \
...
async def check_billing(context: ContextTypes.DEFAULT_TYPE):
    ...
    for account_id in AD_ACCOUNTS:
        try:
            ...
            await context.bot.send_message(chat_id=CHAT_ID, text=f"⚠ Ошибка: 'account_status'", parse_mode='HTML')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = [['Сегодня', 'Вчера', 'Прошедшая неделя']]
    await update.message.reply_text('🤖 Выберите отчёт:', reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
