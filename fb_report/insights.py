...
# === INSIGHTS / HEATMAP ===
from fb_report.insights import (
    load_local_insights,
    save_local_insights,
    extract_actions,
    _blend_totals,
...
async def send_period_report(ctx, chat_id, period, label: str = ""):
    ...
    за остальные периоды — через кеш (get_cached_report).
    """
    store = load_accounts()

    for aid in get_enabled_accounts_in_order():
        # двойная проверка enabled, чтобы жёстко отсечь выключенные
        if not store.get(aid, {}).get("enabled", True):
            continue

        # За сегодня — только live-данные
        if period == "today":
...
def fetch_adset_insights_7d(aid: str):
    ...
    for row in data:
        ...
        # CPA кампании будем считать позже, от суммы
        camp["adsets"].append(
            {
                "id": adset_id,
                "name": adset_name,
...
async def send_adset_report(ctx: ContextTypes.DEFAULT_TYPE, chat_id: str, aid: str):
    """
    Шлём отчёт по адсетам:
    - Сначала блоки по кампаниям (название + метрики за 7 дней)
    - Затем отдельные сообщения по каждому адсету:
      "Кампания - Адсет - метрики" + кнопки управления
    Кнопки используют тот же автопилот (ap|...), что и рекомендации.
    """
    ...
    for camp in campaigns:
        ...
        for ad in camp["adsets"]:
            ...
            # Кнопки: редактировать бюджет / выключить — через автопилот
...
# =========================
# 📌 ПРАВИЛЬНАЯ ФУНКЦИЯ cmd_heatmap
# =========================
...
async def on_cb_autopilot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 2) Выбор основного режима (Рекомендации / Автопилот)
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("one_today|"):
        ...
        # Сегодня — только live-отчёт без текстового кеша
...
async def on_text_any(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # если ни один режим ввода не ожидается — просто молчим
    return
...
def build_app() -> Application:
    ...
    # сначала автопилот (pattern "^ap"), потом общий on_cb
